...
export async function getAllPdfsFromProject(projectId: string): Promise<kronosKnowledgeBaseType[] | null>{
  ...
  export async function getAllPdfs(): Promise<ProjectType[] | null>{
    ...
    try{
      ...
      const result: ProjectType[] = allPdfs.map((pdf: kronosKnowledgeBaseType) => ({
        name: pdf.name,
        lastUpdate: pdf.chatbot_config.attributes?.last_update || formatKronosDate(new Date(pdf.created_at)),
